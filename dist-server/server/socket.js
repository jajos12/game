...
export function setupSocketHandlers(io) {
    io.on('connection', (socket) => {
        ...
        socket.on(SOCKET_EVENTS.SELECT_CARD, (cardId) => {
            const room = findRoomBySocketId(socket.id);
            ...
            if (!player)
                return;
            // Check lockout
            if (Date.now() < player.lockedUntil)
                return;
            const card = room.board.find((c) => c.cardId === cardId);
            ...
            if (playerSelections.includes(cardId))
                return;
            ...
            if (playerSelections.length === 2) {
                ...
                const card1 = room.board.find((c) => c.cardId === id1);
                const card2 = room.board.find((c) => c.cardId === id2);
                ...
                if (isMatch) {
                    ...
                    player.wrongStreak++;
                    let penalty = 1;
                    if (player.wrongStreak === 2)
                        penalty = 2;
                    else if (player.wrongStreak >= 3)
                        penalty = 5;
